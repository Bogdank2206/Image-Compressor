...
app.post(
    ...
    async (req: FileRequest, res: Response) => {
        ...
        if (isNaN(qualityNumber) || qualityNumber < 0 || qualityNumber > 100) {
            return res.status(400).json({code: 1, message: 'Invalid quality value. Must be number between 0 and 100.'});
        }
        ...
        if (!['webp', 'jpg'].includes(format)) {
            return res.status(400).json({
                code: 1,
                message: 'Invalid quality value. Must be number \'webp\' or \'jpg\''
            });
        }
        ...
        const {code, compressedFilename} = await FileService.compressFile(workDir, filename, quality, format);
        if (code === 1 || typeof compressedFilename === 'undefined') {
            return res.status(500).json({code, message: 'Something broke!'});
        }

        const compareService = new CompareService(workDir, filename, compressedFilename, rows, cols);
        const {x: zoneX, y: zoneY, err, diff} = await compareService.compareFiles();
        const {originalName} = req;

        const sizes: string[] = ['B', 'KB', 'MB'];
        let i: number;
        let diffNumber: number = diff;
        for (i = 0; i < sizes.length && diffNumber >= 1024; i++, diffNumber /= 1024) {
        }
        ...
        await promisify(fs.appendFile)(
            ...
            `${API_LINK}/${folderHash}/compressed/${compressedFilename};${JSON.stringify(metadata)}\n`,
        );

        res.status(200).render("imageWithStyles", {
...
app.get(
    ...
    (req: Request, res: Response) => {
        ...
        try {
            const content: string = fs.readFileSync(path.join('public', folderHash, 'files.txt'), 'utf8');
