...
class FileService {
    public async compressFile(workDir: string, filename: string, quality: string, format: FileFormats) {
        ...
        const pathToCompressed: string = path.join(folderOfCompressed, compressedFilename)

        try {
            await execPromise('magick', [pathToFile, "-quality", quality, pathToCompressed]);
