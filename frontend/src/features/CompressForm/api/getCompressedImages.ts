...
export function getCompressedImages(files: FileList | undefined | null, quality: number, format: string, rows: number, cols: number, folder: string): response {
    ...
    const pageCreators = Array.from(files).map((file: File) => async (): Promise<Metadata> => {
