...
export function CompressForm() {
    ...
    const inputFolder: RefObject<HTMLInputElement | null> =
    ...
    function changeQuality({
        ...
        if (
            (!isNaN(quality) && 0 <= quality && quality <= 100) ||
            value === ""
        ) {
            setQuality(quality);
        }
    ...
    async function submit() {
        ...
        const { pageCreators, code, message } = getCompressedImages(
            inputFolder.current?.files,
            quality,
            format,
            rows,
            cols,
        ...
        try {
            const promises = pageCreators.map(
                async (creator: () => Promise<Metadata>) => {
    ...
    return (
        <Box sx={styles}>
            ...
            {wait !== -1 && (
                <Box>
                    {String(Math.floor(wait / 60)).padStart(2, "0")}:
                    {String(wait % 60).padStart(2, "0")}s
                </Box>
            )}
            {loaded && (
                <Button
